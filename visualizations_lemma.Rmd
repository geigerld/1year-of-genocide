...
ggplotly(data_long %>% filter(variable==c("..Israel","..Palestine","..Israeli","..Palestinian")) %>%  ggplot(aes(x = publish_date, y = rel_acccum_freq, color = variable)) +
    geom_line()+
...
data_cor <- cor(use = "complete.obs", method = "pearson", x=data_long$publish_date,y=data_long$rel_acccum_freq)

cor(AJ$..Israeli, BBC$..Israeli)
```



```{r}
plot_outlet <- function(outlet, date_to_plot) {
  filtered_data <- combined_channels %>% filter(media==outlet)
  for (name in search_terms){
    filtered_data <-  add_accum(name, combined_channels)
    }
  plot1 <- filtered_data %>%
    ggplot(aes(x = publish_date, y = acccum_freq, color = media)) +
    geom_line() +
